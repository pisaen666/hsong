...
## 9. ศูนย์จัดของฮับ & การจ่ายงานด่วนแผงค้า (Hub Express Dispatch Operations v=9.10)

ในหน้าจอของทีมงานจัดของฮับส่วนกลาง (`#hub-view-container` แถบ `📋 จัดของ`) เมื่อแผงค้ามีการเรียกไรเดอร์ด่วน การ์ดงานด่วนจะปรากฏขึ้นพร้อมเครื่องมือควบคุมการจ่ายงานที่ใช้งานได้จริง 100%:

### 1. เครื่องมือติดต่อสื่อสารด่วน (Quick Direct Contact System):
* **📞 โทรหาแผงค้า (`callContactDirect`):**
  * แสดงเบอร์โทรและชื่อร้านค้าต้นทางอย่างชัดเจน
  * แตะเพื่อโทรออกทันทีบนมือถือ หรือเปิดหน้าต่างโมดอลพร้อมปุ่มโทร/คัดลอกเบอร์บนคอมพิวเตอร์
  * แก้ไขข้อความชื่อแผงค้าซ้ำซ้อน เช่น `ร้านไก่สดเฮียส่ง (แผง A01)` ให้กระชับ ไม่เบิ้ลวงเล็บ
* **📞 โทรหาลูกค้า (`callContactDirect`):**
  * เชื่อมต่อเบอร์โทรลูกค้าปลายทางทันที
  * จัดการแสดงระยะทางให้เป็นทศนิยม 1 ตำแหน่งเสมอ เช่น `~1.2 กม.` (ตัดทศนิยมยาวเกินจำเป็น)

### 2. เครื่องมือจ่ายงานและจัดสรรไรเดอร์ (Dispatch & Assignment Controls):
* **กรณีงานยังไม่ถูกจ่าย (รอกำลังพล):**
  * 🔘 **`🚀 จ่ายงานให้ไรเดอร์สมศักดิ์ (พร้อมรับงาน)`:** ปุ่มด่วน 1-Click สำหรับจ่ายงานให้ไรเดอร์ประจำฮับทันที งานจะเปลี่ยนสถานะเป็น `assigned` พร้อมแจ้งเตือนไปยังหน้าจอไรเดอร์และร้านค้าแบบเรียลไทม์
  * 🔘 **`👥 เลือกไรเดอร์คนอื่น` (`openAssignRiderModal`):** เปิดโมดอลแสดงรายชื่อไรเดอร์ทั้งหมดในเครือข่ายฮับ (พร้อมแสดงสถานะ ออนไลน์/พร้อมรับงาน) ให้ผู้จัดการฮับเลือกจ่ายงานให้ไรเดอร์คนใดก็ได้
  * 🔘 **`🖨️ พิมพ์สลิปส่งด่วน 80mm` (`printMerchantExpressSlip`):** สั่งพิมพ์สลิปขนาด 80mm ทันทีเพื่อนำไปแปะหน้าถุงสินค้าหรือส่งให้ไรเดอร์
* **กรณีงานจ่ายไรเดอร์แล้ว (จ่ายงานแล้ว / กำลังส่ง):**
  * 🔘 **`📞 โทรหาไรเดอร์`:** โทรประสานงานกับไรเดอร์ที่รับผิดชอบงานได้โดยตรง
  * 🔘 **`📡 ดูเรดาร์สด GPS` (`viewOrderOnRadar`):** เปิดดูพิกัดสดของไรเดอร์บนแผนที่ดาวเทียมจากหน้าจอฮับได้ทันที
  * 🔘 **`🔄 เปลี่ยนไรเดอร์` (`openAssignRiderModal`):** สลับหรือโอนงานให้ไรเดอร์คนอื่นกรณีไรเดอร์เดิมติดธุระ
  * 🔘 **`🖨️ สลิป 80mm`:** พิมพ์ซ้ำได้ตลอดเวลา
  * 🔘 **`✅ บันทึกส่งสำเร็จ` (`markExpressDeliveredByHub`):** ผู้จัดการฮับสามารถกดยืนยันปิดงานส่งสำเร็จได้ทันทีเมื่อได้รับการยืนยันทางวิทยุหรือโทรศัพท์

### 3. ปุ่มระบบส่วนหัว & การสลับแท็บงาน (Top Header & Hub Tab Switching):
* **`🗑️ ล้างออเดอร์` (`clearAllTestData`):** ล้างออเดอร์ทดสอบทั้งหมด รวมถึง Express Orders และคืนค่าคิวเป็น 0 ทันที โดยมีระบบความปลอดภัยบังคับใส่รหัสผ่านล็อกอินประจำฮับ (`hb6305` หรือรหัส Admin) ก่อนดำเนินการเสมอเพื่อป้องกันการลบข้อมูลโดยไม่ได้ตั้งใจ
* **`🔄 ซิงค์สด` (`syncLatestOrderFromCloud`):** ดึงข้อมูลออเดอร์ล่าสุดทั้งจากคลาวด์ Firebase และหน่วยความจำในเครื่องมาอัปเดตหน้าจอทันที
* **`🚪` (`logoutHub`):** ออกจากระบบจัดการจัดส่งฮับ และสลับกลับไปหน้าจอลูกค้าอย่างปลอดภัย
* **`📋 จัดของ` / `💰 เคลียร์เงิน` / `📊 ติดตามออเดอร์` / `📑 รายงานประจำวัน DB`:** สลับแท็บพร้อมเรนเดอร์ข้อมูลแบบ Real-time ทันทีทุกครั้งที่แตะ

### 4. แท็บ 💰 เคลียร์เงิน (Hub Settlement Tab):
* **สรุปยอดจ่ายแผงค้า (Grocery Orders):** แสดงรายการแผงค้าและยอดเงินที่ต้องโอน พร้อมปุ่ม `[โอนเคลียร์เงิน]` แบบโต้ตอบทันที
* **สรุปค่าจัดส่งด่วน (Express Fleet):** แสดงรายการงานด่วน ค่าส่ง ฿20 (โอนเข้าฮับ) พร้อมปุ่มบันทึกสถานะการโอนค่ารอบให้ไรเดอร์
