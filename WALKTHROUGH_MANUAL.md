...
### หมวดที่ 4: บริหารงานไรเดอร์ & เรดาร์ GPS สด (Riders Fleet Operations)
> 🚀 **ฟีเจอร์ระดับปฏิบัติการที่พัฒนาล่าสุด (v=6.8)**

1. **4 การ์ด Live KPIs สรุปสถานะฟลีตด่วน**:
   * จำนวนไรเดอร์ทั้งหมด (แยกว่างกี่คน, ส่งของกี่คน, พักกี่คน)
   * จำนวนรอบส่งสำเร็จวันนี้ + ค่ารอบรวมสะสม
   * ยอดเงินสด COD ในมือรวมทั้งหมดที่รอการนำส่งเข้าฮับ
   * สถานะสภาพอากาศและอัตราโบนัสปัจจุบัน
2. **สลับสถานะ 1-Click (Instant Status Switcher)**:
   * ที่การ์ดไรเดอร์แต่ละคน มีปุ่มลัด `[พร้อม]`, `[ส่งของ]`, `[พัก]`
   * เปลี่ยนสถานะได้ทันที พร้อมเปลี่ยนสีหมุดบนแผนที่เรดาร์ และบันทึกลงฐานข้อมูล
3. **ระบบเฝ้าระวังเงินสด COD ในมือ (Cash In-Hand Safety Limit)**:
   * ตรวจจับยอดเงินสด COD ที่ไรเดอร์ถืออยู่
   * หากยอดเกินเกณฑ์ปลอดภัย (ตั้งไว้ ฿2,500) การ์ดจะแสดงป้ายเตือนสีแดงกะพริบ `⚠️ เงินสดในมือเกินเกณฑ์!`
   * ปุ่ม **"เคลียร์เงิน COD"**: กดยืนยันรับเงินสดเข้าฮับ ยอดเงินสดในมือจะตัดกลับเป็น ฿0 ทันที
4. **เรดาร์แผนที่ไรเดอร์สด (Live Radar GPS Leaflet Map)**:
   * แผนที่แสดงพิกัดรอบตลาดสดวิศิษฐ์ชัย ชุมชนหนองชาก และอำเภอบ้านบึง
   * หมุดศูนย์กลางฮับ (`🏛️`)
   * หมุดไรเดอร์ตามสีสถานะ: 🟢 พร้อมรับงาน | 🟡 กำลังส่งของ | 🔴 พักรอบ
   * คลิกที่หมุดเพื่อดูข้อมูลด่วนและกดปุ่มโทรออกหาไรเดอร์ได้ทันที
5. **แผงปรับแต่งค่ารอบ & โบนัสความเร็ว (Dynamic Compensation)**:
   * **สวิตช์ฝนตก 1-Click (`🌧️`)**: กดเพื่อเปิดโหมดสภาพอากาศแย่ เพิ่มค่ารอบพิเศษ (+฿15/เที่ยว หรือตามกำหนด) ให้ไรเดอร์ทุกคนทันที
   * ปรับค่ารอบมาตรฐาน (฿/เที่ยว)
   * กำหนดเป้าหมายโบนัสความขยัน (เช่น วิ่งครบ 10 เที่ยว +฿100)
   * กำหนดเพดานวงเงินสด COD สูงสุด
6. **เพิ่ม / แก้ไข / ลบ ข้อมูลไรเดอร์ (Full CRUD Modal)**:
   * ปุ่ม **"+ เพิ่มไรเดอร์ใหม่"** ด้านบน เปิด Modal กรอกข้อมูล
   * รองรับการแก้ไขชื่อ, เบอร์โทร, ทะเบียนรถ, โซนพื้นที่ และการลบไรเดอร์
...
|---|---|---|
| [`index.html`](file:///c:/pisaen/hsong/index.html) | โครงสร้างหน้าเว็บหลักและ Modal ทั้งหมด | โครงสร้าง Admin Widescreen Console (บรรทัด ~1130), Modal ไรเดอร์ `#rider-form-modal` (บรรทัด ~3185), เวอร์ชันสคริปต์ `app.js?v=6.8` |
| [`app.js`](file:///c:/pisaen/hsong/app.js) | ตรรกะการทำงาน (Business Logic) และข้อมูล | ฟังก์ชันคำนวณสลิปความร้อน `printThermalRiderSlip`, ตรรกะฟลีตไรเดอร์ `renderAdminRiders`, แผนที่เรดาร์ `initAdminRiderRadarMap`, การสลับแท็บ `switchAdminTab` |
| [`styles.css`](file:///c:/pisaen/hsong/styles.css) | การจัดสไตล์และ CSS สำหรับการพิมพ์ | การล็อก `overflow-x: hidden`, การกำหนดสไตล์การพิมพ์ `@media print` และ `@page` ขนาด 68-72mm สำหรับ POS Slip |
