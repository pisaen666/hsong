...
- Rider login (`handleRiderPhoneLoginSubmit`, `handleOnPageRiderLoginSubmit`) still accepts a partial rider NAME as the login (`name.includes(...)`), so a short guess like "a" can log in as the first matching rider. Not fixed yet.
