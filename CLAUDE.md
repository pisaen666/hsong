...
- **Rollout order (do not swap):** (1) DONE 2026-09-21: publish `hsong-1f342.rules.v2.json` (v2.2; compatible with the live code because no hash exists yet) with the owner's OK, (2) DONE 2026-09-22: push the code (owner phrase), (3) owner signs in on the real site and presses 🔑 for each REAL stall and sends the password. On 2026-09-22 the owner said `APP-SHOP-6758` (ร้าน ส.ไก่สด) is HIS OWN TEST stall and may be deleted; `stall_test_chicken` is also a test stall (name says delete). Deleting needs the owner signed in (Claude cannot), so the owner deletes them in the admin stall list (trash icon on the card). Until step 3 real merchants cannot log in (old codes stop working by design). Rollback file for rules = git `HEAD~` version of `hsong-1f342.rules.v2.json` (v2.1).
