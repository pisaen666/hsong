...
## 🛡️ 5c. Rules v2 & per-item cloud writes (written 2026-09-21, tested on hsong-test only)
Goal: anyone may APPLY, only the owner may APPROVE / create real riders & stalls / delete. Files: `firebase-rules/build-rules.js` generates `hsong-test.rules.v2.json` and `hsong-1f342.rules.v2.json` (edit the template, never the JSON). The old `*.rules.json` files are what is published now.
...
- PRODUCTION rules (`firebase-rules/hsong-1f342.rules.json`, published) still expire 2026-11-02 (`1793552400000`) and leave orders, carts, rider_applications, community_riders, merchant_applications etc. open to everyone until then. The replacement (rules v2, section 5c) is written and tested on `hsong-test` but NOT published to production yet.
- Rider login (`handleRiderPhoneLoginSubmit`, `handleOnPageRiderLoginSubmit`) accepts only access code / rider id / phone number — matching by NAME was removed 2026-09-21 (names are public). Phone numbers are still readable in `community_riders`, so phone-only login is weak; a proper fix needs a secret code (or rider Firebase Auth). Test: `firebase-rules/tests/rider-login.test.js`.
- Public-read leftovers: rider/merchant application records still carry `accessCode` (= the login code) and are readable by anyone; old records may still hold private fields (address, bank) on the cloud (the client never deletes them because `rider_private` might not have a copy). Orders/carts contain customer data and stay open until customers get Firebase Auth.
